...
# Global hotkey via pynput (cross-platform)
try:
    from pynput import keyboard as pynput_keyboard
    PYNPUT_AVAILABLE = True
except ImportError:
    PYNPUT_AVAILABLE = False
    print("Warning: pynput not installed. Overlay hotkeys disabled. Install with: pip install pynput")
...
class HotkeyManager:
    """Global hotkey manager via pynput (double-press detection, cross-platform)"""
    ...
    def start(self):
        if not PYNPUT_AVAILABLE:
            print("[HotkeyManager] pynput not available, hotkeys disabled", flush=True)
            return
        self._listener = pynput_keyboard.Listener(on_press=self._on_press)
        self._listener.daemon = True
        self._listener.start()
        print("[HotkeyManager] Listening. F1x2=hide, F2x2=show, F3x2=quit", flush=True)
...
# ═══════════════════════════════════════════════════════════════
# Helper: get audio sources via sounddevice (cross-platform)
# ═══════════════════════════════════════════════════════════════

def get_audio_sources():
    """Get list of audio sources using sounddevice (works on Windows/macOS/Linux)"""
    loopback_devices = []
    input_devices = []
    try:
        ...
        for i, d in enumerate(devices):
            if d['max_input_channels'] > 0:
                name = d['name']
                # Detect loopback/stereo mix devices (system audio capture)
                loopback_keywords = ['stereo mix', 'loopback', 'what u hear',
                                     'wave out', 'monitor', 'virtual']
                is_loopback = any(kw in name.lower() for kw in loopback_keywords)
                if is_loopback:
                    friendly = f"System Audio ({name})"
                    loopback_devices.append((friendly, i))
                else:
                    friendly = f"Microphone ({name})"
                    input_devices.append((friendly, i))
    except Exception:
        pass
    return loopback_devices + input_devices
...
class ConfigWindow(QMainWindow):
    ...
    def get_device_config(self):
        ...
        # On Windows, sounddevice uses device index directly — no need for pactl
        # source_name from get_audio_sources() is already a device index (int)
        device_index = source_name  # This is the sounddevice device index
        try:
...
class OverlayWindow(QWidget):
    ...
    def setup_stealth(self):
        """Set up stealth mode: Windows screen capture exclusion via SetWindowDisplayAffinity"""
        QTimer.singleShot(500, self._apply_win32_stealth)

    def _apply_win32_stealth(self):
        """Apply Windows SetWindowDisplayAffinity to exclude from screen capture.

        WDA_EXCLUDEFROMCAPTURE (0x11) makes the window invisible to all
        screen capture tools including Google Meet, OBS, and PrintScreen.
        The window remains visible to the local user on their monitor.

        This is the Windows equivalent of the Linux screen capture exclusion
        problem — except on Windows it actually works reliably.
        """
        if not WIN32_AVAILABLE:
            print("[Stealth] Win32 API not available (not running on Windows)")
            return
        try:
            hwnd = int(self.winId())
            result = user32.SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)
            if result:
                print("[Stealth] SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE) applied — overlay is invisible to screen capture")
            else:
                # Fallback to WDA_MONITOR (older Windows 10)
                WDA_MONITOR = 0x00000001
                result = user32.SetWindowDisplayAffinity(hwnd, WDA_MONITOR)
                if result:
                    print("[Stealth] SetWindowDisplayAffinity(WDA_MONITOR) applied — overlay shows black in screen capture")
                else:
                    print(f"[Stealth] SetWindowDisplayAffinity failed (error code: {ctypes.GetLastError()})")
        except Exception as e:
